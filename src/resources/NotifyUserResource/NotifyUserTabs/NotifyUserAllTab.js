...
import { PopupCommon, SearchInput, TableGridCommon } from '@utilities';
...
const NotifyUserAllTab = () => {
  ...
  const handleTrash = useCallback(
    (id) => () => {
      dispatch(trashNotifyUserAction(id, whoami?.id, queryOptions));
    },
      ...
      },
      {
        ...
        getActions: (params) => {
          return [
            ...
            <GridActionsCellItem
              key={params.id}
              label={translate('common.label.trash')}
              showInMenu
              icon={<RestoreFromTrashIcon />}
              onClick={handleTrash(params.id)}
            />
          ];
        }
      }
    ];
  }, [handleView, handleTrash, queryOptions, translate, i18n.language]);
...
export default NotifyUserAllTab;
